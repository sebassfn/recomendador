...
"¿y si el modelo alucina un producto?": no puede, porque no elige productos.
...
| 7 | Atributo requerido | falta una clave de `slot.required_attributes` | `MISSING_REQUIRED_ATTRIBUTE` | Se relaja a advertencia si la cobertura del atributo en el catálogo < 50 %. |
...
> reintenta una vez. El slot que igual no se resuelve se muestra vacío con el motivo
> — **un slot vacío honesto vale más que una canasta que miente sobre el total.**
...
## 6. Tres ejemplos numéricos resueltos
...
- Se muestra como "+S/ 84 · +47 % de ticket · 3 categorías cubiertas".
